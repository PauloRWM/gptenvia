    ...
    const [resources, setResources] = useState([
        {
            ...
            name: "Vídeos",
            active: false,
            description: "Inclui vídeos explicativos no treinamento",
        },
        {
            id: 2,
            name: "Quizzes",
            active: false,
            description: "Adiciona questionários interativos",
        },
        {
            id: 3,
            name: "Fórum",
            active: false,
            description: "Habilita um fórum de discussão para os participantes",
        },
        {
            id: 4,
            name: "Certificado",
            active: false,
            description: "Emite um certificado ao concluir o treinamento",
        },
    ]);
...
const fetchTrainingData = async () => {
        try {
            ...
            setResources(data.resources ? JSON.parse(data.resources) : []);
            setChannels(data.channels || [
                { id: 1, name: "WhatsApp", status: "disconnected" },
            ]);
        } catch (error) {
            console.error("Erro ao buscar os dados do treinamento:", error);
        }
    };
    ...
    const handleSave = () => {
        ...
        console.log(
            "Active resources:",
            resources.filter((r) => r.active).map((r) => r.name)
        );
                    ...
                    <CardContent>
                        ...
                        {resources.filter((r) => r.active).length === 0 && (
                            <p className="text-muted-foreground">
                                Nenhum recurso ativo no momento.
                            </p>
                        )}
                    </CardContent>
                </Card>
                                    ...
                                    <DialogContent className="sm:max-w-[425px]">
                                        ...
                                        <div className="flex flex-col items-center justify-center p-4">
                                            ...
                                            <p className="text-center text-sm text-muted-foreground">
                                                Escaneie o QR code acima com o
                                                seu {channel.name} para
                                                reconectar.
                                            </p>
                                            <Button
                                                className="mt-4"
                                                onClick={() =>
                                                    handleReconnect(channel.id)
                                                }
                                            >
                                                Confirmar Reconexão
                                            </Button>
